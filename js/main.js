...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function setupMenuListeners() {
    // Basic
    document
      .getElementById("add_entity")
      .addEventListener("click", () => createElement("entity"));
    document
      .getElementById("add_attribute")
      .addEventListener("click", () => createElement("attribute"));
    document
      .getElementById("add_relationship")
      .addEventListener("click", () => createElement("relationship"));
    document
      .getElementById("add_label")
      .addEventListener("click", () => createElement("label"));

    // College Features
    document
      .getElementById("add_attr_multi")
      .addEventListener("click", () => createElement("attribute-multi"));
    document
      .getElementById("add_attr_derived")
      .addEventListener("click", () => createElement("attribute-derived"));
    document
      .getElementById("add_isa")
      .addEventListener("click", () => createElement("isa"));
    document
      .getElementById("generate_sql")
      .addEventListener("click", generateSQL);

    // Toggles
    document.getElementById("toggle_pk").addEventListener("click", () => {
      state.selectedElements.forEach((el) =>
        el.classList.toggle("primary-key"),
      );
    });
    ...
    document
      .getElementById("generate_schema")
      .addEventListener("click", generateSchema);
    ...
    document
      ...
      .addEventListener("click", () => zoomCenter(0.1));
    document
  ...
  function updateButtonStates() {
    const lineBtn = document.getElementById("add_line");
    const deleteBtn = document.getElementById("delete_mode");
    if (state.lineMode) lineBtn.classList.add("active");
    else lineBtn.classList.remove("active");
    if (state.deleteMode) deleteBtn.classList.add("delete-active");
  ...
  function setupViewportControls() {
    ...
    viewport.addEventListener("mousedown", (e) => {
      ...
      if (
        e.button === 0 &&
        (e.target === viewport || e.target === contentLayer)
      ) {
  ...
  function getDiagramJSON() {
    ...
    document.querySelectorAll(".element").forEach((el) => {
      data.elements.push({
        ...
        text:
          el.dataset.type === "schema"
            ? el.querySelector(".element-text").innerHTML
            : el.querySelector(".element-text").innerText,
